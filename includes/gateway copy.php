...
class PayInvert extends WC_Payment_Gateway
{
    public function __construct()
    {
        ...
        // AJAX callback for regenerating auth header value
        add_action('wp_ajax_payinvert_regenerate_auth_header_value', array($this, 'ajax_regenerate_auth_header_value'));
    }
    ...
    // Add AJAX callback for regenerating auth header value
    public function ajax_regenerate_auth_header_value()
    {
        check_ajax_referer('payinvert_regenerate_auth_nonce', 'security');
        ...
        wp_send_json_success('Auth header value regenerated successfully');
    }

    // Display the button to regenerate auth header value in the settings page
    public function add_auth_header_settings()
    {
        if (!current_user_can('manage_woocommerce')) {
            return;
        }
        ?>
        <h2>
            <?php esc_html_e('PayInvert Gateway Settings', 'payinvert-gateway'); ?>
        </h2>
        <table class="form-table">
            <?php $this->generate_settings_html(); ?>
            <tr valign="top">
                <th scope="row" class="titledesc">
                    <?php esc_html_e('', 'payinvert-gateway'); ?>
                </th>
                <td class="forminp">
                    <button type="button" class="button regenerate-auth-header-value-button">
                        <?php esc_html_e('Regenerate Auth Header Value', 'payinvert-gateway'); ?>
                    </button>
                    <p class="description">
                        <?php esc_html_e('Click this button to regenerate the Auth Header Value.', 'payinvert-gateway'); ?>
                    </p>
                </td>
            </tr>
        </table>
        <script>
            jQuery(document).ready(function ($) {
                $('.regenerate-auth-header-value-button').on('click', function () {
                    const data = {
                        action: 'payinvert_regenerate_auth_header_value',
                        security: '<?php echo wp_create_nonce('payinvert_regenerate_auth_nonce'); ?>'
                    };
    
                    $.post(ajaxurl, data, function (response) {
                        $('input[name="woocommerce_payinvert_settings[auth_header_value]"]').val(response.data);
                        alert('Auth header value regenerated successfully');
                    });
                });
            });
        </script>
